function ListMenu (settings) {
  ...
  function recurse_gen (obj) {
   ...
   list_items.forEach(function (list_item) {
    ...
    if (settings_touch_events == true) {
     list_item.addEventListener('touchstart', show_event)
    } else {
 ...
 function clear_queued_events () {
  queued_events.forEach(function (queued_event) {clearTimeout(queued_event)})
  queued_events = []
 }
 ...
 function hide_event (evt, hide_on_click, is_immediate) {
  ...
  function hide_event_cont () {
   ...
   container_list.forEach (function (container) {
    var hide_list = container.listmenu_settings.parent ? hide_children(container.listmenu_settings.parent) : hide_grandchildren(container, 0)
 ...
 function show_event (evt, show_on_click, is_immediate) {
  ...
  function show_event_cont () {
   ...
   var hide_list = []
   list_item.listmenu_settings.parent.listmenu_settings.children.forEach (function (sibling) {
    if (sibling != list_item) hide_list = hide_list.concat(hide_children(sibling))
   })
 ...
 function hide_grandchildren (container, level) {
  var new_level = (typeof level != "undefined") ? level + 1 : undefined
  var children = container.listmenu_settings.children
  var descendant_list = []
  children.forEach (function (list_item) {
 ...
 function begin_process (showhide_list, showhide, options) {
  showhide_list.forEach (function (entry) {
   if (typeof entry.listmenu_settings.parent == "undefined") return
   entry.listmenu_settings.parent.listmenu_settings.is_open = (showhide == 'show') ? true : false
  })
  ...
  var j = 0
  showhide_list.forEach(function (obj) {
   ...
   if ((showhide == 'hide') && (obj.style.display == 'none')) return
 ...
 function click_event (evt)  {
  ...
  list_item.listmenu_settings.parent.listmenu_settings.children.forEach(function (sibling) {
   if (sibling != list_item) clear_selected(sibling)
  })
 ...
 function hide_ancestors_and_self (container, hide_list) {
  ...
  return hide_ancestors_and_self(container.listmenu_settings.parent.listmenu_settings.parent, hide_list)
 }
...
ListMenu.anim_fade = function (obj_list, counter, showhide) {
 obj_list.forEach (function (obj) {
  ...
  if (showhide != "show") {if (counter == 0) obj.style.display = "none"; return}
